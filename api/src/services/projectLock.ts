...
// that disagree deadlock as soon as their sets overlap, which random ids make
// about half of all pairs. The key is the order because every locker already
// has it; a list wanting some other order can be sorted once the rows are held.
...
// commit mid-request and leave the new creator holding a member row. Writers of
// a project's invitations hold it too, so that a revoke cannot land inside a
// re-invite that is about to recreate the row it revoked, id and link alike.
