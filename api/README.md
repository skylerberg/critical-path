...
committed, and only the generated half changes here.
...
belong to the commit hook the root README describes. That is also why this one
only means anything once the tree is committed.
...
There is no `openapi:dump` step in front of it — every generator re-dumps for
itself. `codegen-ci.yaml` re-runs the script on a pull request and rejects one
whose committed clients are not what it produced.
