// Deprecated operations and schemas are removed from the spec before codegen, so
// a stale call site in either client turns into a type error rather than
// compiling against an endpoint the API has already stopped promising.
...
// Removing a deprecated operation orphans every schema only it referenced, and
// openapi-typescript emits components.schemas whole, reachable or not. Without
// this pass the client still exports the request and response types of an
// endpoint it can no longer call, and a call site can still be written against
// them — which is the thing filtering exists to prevent.
...
/** Mutates `spec`, and reports what it took out. */
export function filterDeprecated(spec) {
  let removedOps = 0;
