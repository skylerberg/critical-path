...
`cpath` is what this package builds, so everything below is a description of
`src/` in this checkout rather than of an installed release: a flag added here
is available the moment `bin/` resolves to this tree, and one removed here stops
working for every agent at once.

It is a full client for Critical Path, and **it defaults to production**
(`https://criticalpath.skylerberg.com`), not localhost — every write lands on
the live board, so confirm the target project and task before mutating.
