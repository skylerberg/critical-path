...
which is the capability the merge exists to provide. `api/CLAUDE.md` has the
...
dumps beside its OpenAPI spec.

**Neither needs a dump first**, and neither is silent about what it read.
`scripts/spec-source.mjs` resolves the api package at a fixed in-repo path,
re-dumps it, and prints the absolute path it used. There is no search and no
ancestor walk any more, so the whole class of bug where a worktree generated
against some other checkout and wrote a client with no diff cannot happen: the
sources it reads are the ones this branch is changing. A missing `api/` is a
